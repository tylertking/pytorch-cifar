...
def augmented_min(X, max_iter=10, rtol=1e-6, atol=1e-6):
    minimum = torch.mean(X, dim=0, keepdim=True).item()

    for _ in range(max_iter):  

        dims = tuple(list(X.size()))
        
        min_arr = torch.full(dims, float(minimum)).to(X.device)
        
        # f(x_0) sum
        numerator = torch.sum( torch.pow(torch.abs( torch.sub(min_arr, X) ), 1.5) )
        # f'(x_0) sum
        denominator = 3/2*torch.sum( torch.pow(torch.abs( torch.sub(min_arr, X) ), 0.5) )
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                ...
                BatchNorm(self.expansion*planes, 4)
            )
